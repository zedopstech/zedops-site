...
export default function BlogMainCategories({MAIN_CATEGORY_DATA}: {MAIN_CATEGORY_DATA: any}) {
  return (<>
    ...
    <div className="container">
        <div className="mb-8 flex flex-row items-center justify-between">
            ...
            <a className="group" href={MAIN_CATEGORY_DATA.viewAllLink}>
                <span
                    className="relative inline-flex items-center gap-1 leading-none after:absolute after:-bottom-1.5 after:left-0 after:h-[2px] after:w-full after:bg-foreground after:opacity-0 after:transition-all after:ease-out-fast after:group-hover:-bottom-1 after:group-hover:opacity-100 group-active:after:-bottom-1"
                >
        ...
        <div className="grid grid-cols-1 gap-3">
            <article
                className="group relative flex flex-1 cursor-pointer rounded-sm border p-3 transition-all ease-out translate-y-0 hover:border-foreground/10 hover:bg-background-darker/60 hover:-translate-y-0.5 dark:hover:bg-background-lighter/50 md:flex-row flex-col gap-4"
            >
                <a aria-label="The PLG playbook: Unlocking growth by letting your product shine" className="absolute inset-0 z-0" href={MAIN_CATEGORY_DATA.featuredArticle.link}></a>
                <a className="relative z-10 w-full self-start overflow-hidden rounded-sm sm:aspect-[16/10] aspect-[16/10] max-w-full sm:max-w-full sm:flex-1" href={MAIN_CATEGORY_DATA.featuredArticle.link}>
                    <img
                    ...
                    <div className="flex h-full flex-col items-start justify-between gap-8 px-2 md:px-3 md:py-2">
                        <div className="flex w-full flex-col gap-1 sm:gap-2">
                            <div className="flex h-5 flex-row items-center justify-between">
                                <a className="relative z-10 text-sm font-semibold text-accent decoration-1 underline-offset-2 hover:underline" href={MAIN_CATEGORY_DATA.featuredArticle.categoryLink}>{MAIN_CATEGORY_DATA.featuredArticle.category}</a>
                            </div>
                            <a href={MAIN_CATEGORY_DATA.featuredArticle.link}><h2 className="text-h6 md:text-h5 lg:text-h4">{MAIN_CATEGORY_DATA.featuredArticle.title}</h2></a>
                            <div className="mt-2">
                        ...
                        <div className="">
                            <div className="flex flex-nowrap items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    <div className="flex flex-row items-center gap-1 text-sm leading-tight"><span className="font-bold">{MAIN_CATEGORY_DATA.featuredArticle.author.name}</span></div>
                                    <div className="text-sm text-foreground-muted/70">{MAIN_CATEGORY_DATA.featuredArticle.author.role}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex flex-1 flex-col gap-3 md:flex-row">
                {MAIN_CATEGORY_DATA.articles.map((article: any, index: any) => (
                    <article
                        key={index}
                        ...
                        <div className="w-full sm:flex-1">
                            <div className="flex h-full flex-col items-start justify-between gap-8 px-1 md:px-2">
                                <div className="flex w-full flex-col gap-1 sm:gap-2">
                                    ...
                                    <a href={article.link}>
                                        <h2 className="text-lg font-semibold leading-[1.15] sm:text-xl sm:leading-[1.15] lg:text-h6">{article.title}</h2>
                                    </a>
                                </div>
                                <div className="hidden md:block">
                                    <div className="flex flex-nowrap items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <div className="flex flex-row items-center gap-1 text-sm leading-tight"><span className="font-bold">{article.author.name}</span></div>
                                            <div className="text-foreground-muted/70 text-sm">{article.author.role}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
